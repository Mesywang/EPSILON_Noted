...
SemanticMapManager::SemanticMapManager(const int& id, const decimal_t surrounding_search_radius,
...
ErrorType SemanticMapManager::UpdateSemanticMap(const double& time_stamp, const common::Vehicle& ego_vehicle,
    ...
    set_obstacle_map(obstacle_map); // GridMapND
    set_obstacle_grids(obstacle_grids); // raycasting障碍物的位置
    ...
    if (agent_config_info_.enable_fast_lane_lut) { // default: true
        // 找到ego_state附近的车道线, 并将车道线合并为local_lane, 构建快速查找表
    ...
    UpdateKeyVehicles(); // semantic_key_vehicles_ and key_vehicles_ not used?

    // * openloop prediction for all semantic vehicles
    if (agent_config_info_.enable_openloop_prediction) { // true
        OpenloopTrajectoryPrediction();
    }

    if (agent_config_info_.enable_log) { // false
        SaveMapToLog();
    }
    return kSuccess;
}
...
ErrorType SemanticMapManager::SaveMapToLog() {
    ...
    for (auto& v : vehicle_set.vehicles) {
        record_file_stream << v.second.state().time_stamp << "," << v.first << "," << v.second.state().vec_position[0]
                           << "," << v.second.state().vec_position[1] << "," << v.second.state().velocity << ","
                           << v.second.state().acceleration << "," << v.second.state().angle << ","
                           << v.second.state().curvature << "," << v.second.state().steer << std::endl;
    }
    return kSuccess;
}

ErrorType SemanticMapManager::NaiveRuleBasedLateralBehaviorPrediction(const common::Vehicle& vehicle,
    ...
    if (use_right_hand_axis_) {
        ...
        if (fs.vec_dt[0] > lat_distance_threshold && fs.vec_dt[1] > lat_vel_threshold
            && nearest_lane.l_lane_id != kInvalidLaneId && nearest_lane.l_change_avbl) {
            prob_lcl = 1.0;
            printf("[NaivePrediction]vehicle %d lane id %d, lat d %lf lat dd %lf, "
                   "behavior "
                   "lcl.\n",
                   vehicle.id(), nearest_lane_id, fs.vec_dt[0], fs.vec_dt[1]);
            // 右手坐标系, 向右距离 > 阈值 && 向右速度 > 阈值 && 右车道线可用 && 右车道线可变道 -> 预测行为: 向右变道
        } else if (fs.vec_dt[0] < -lat_distance_threshold && fs.vec_dt[1] < -lat_vel_threshold
                   && nearest_lane.r_lane_id != kInvalidLaneId && nearest_lane.r_change_avbl) {
            prob_lcr = 1.0;
            printf("[NaivePrediction]vehicle %d lane id %d, lat d %lf lat dd %lf, "
                   "behavior "
                   "lcr.\n",
...
ErrorType SemanticMapManager::MobilRuleBasedBehaviorPrediction(const common::Vehicle& vehicle,
    ...
    std::vector<LateralBehavior> behaviors{LateralBehavior::kLaneKeeping, LateralBehavior::kLaneChangeLeft,
                                           LateralBehavior::kLaneChangeRight};

    for (const auto& behavior : behaviors) {
        // ~ Prepare lane
        common::Lane ref_lane;
        GetRefLaneForStateByBehavior(vehicle.state(), std::vector<int>(), behavior, lane_radius, lane_radius, false,
                                     &ref_lane);
        ...
        GetLeadingAndFollowingVehiclesFrenetStateOnLane(
            ref_lane, vehicle.state(), nearby_vehicles, &has_leading_vehicle, &leading_vehicle, &leading_frenet_state,
            &has_following_vehicle, &following_vehicle, &following_frenet_state);
    ...
    if (common::MobilBehaviorPrediction::LateralBehaviorPrediction(vehicle, lanes, leading_vehicles,
                                                                   leading_frenet_states, following_vehicles,
                                                                   follow_frenet_states, nearby_vehicles, res)
        != kSuccess) {
        return kWrongStatus;
    }
    return kSuccess;
...
ErrorType SemanticMapManager::GetLeadingAndFollowingVehiclesFrenetStateOnLane(
    const common::Lane& ref_lane, const common::State& ref_state, const common::VehicleSet& vehicle_set,
    bool* has_leading_vehicle, common::Vehicle* leading_vehicle, common::FrenetState* leading_fs,
    bool* has_following_vehicle, common::Vehicle* following_vehicle, common::FrenetState* following_fs) const {
...
ErrorType SemanticMapManager::GetEgoNearestLaneId(int* ego_lane_id) const {
    ...
    if (GetNearestLaneIdUsingState(ego_vehicle_.state().ToXYTheta(), std::vector<int>(), &nearest_lane_id, &distance,
                                   &arclen)
        != kSuccess) {
        return kWrongStatus;
    }
...
ErrorType SemanticMapManager::UpdateSemanticVehicles() {
    ...
    for (const auto& v : surrounding_vehicles_.vehicles) {
        ...
        GetNearestLaneIdUsingState(semantic_vehicle.vehicle.state().ToXYTheta(), std::vector<int>(),
                                   &semantic_vehicle.nearest_lane_id, &semantic_vehicle.dist_to_lane,
                                   &semantic_vehicle.arc_len_onlane);

        NaiveRuleBasedLateralBehaviorPrediction(semantic_vehicle.vehicle, semantic_vehicle.nearest_lane_id,
        ...
        decimal_t forward_lane_len = std::max(semantic_vehicle.vehicle.state().velocity * 10.0, 50.0); // 10s行驶的距离
        // 根据lat_behavior获得target_id, 然后获得target_id所对应的lane(cubic spline)
        GetRefLaneForStateByBehavior(semantic_vehicle.vehicle.state(), std::vector<int>(),
                                     semantic_vehicle.lat_behavior, forward_lane_len, max_backward_len, false,
                                     &semantic_vehicle.lane);

        semantic_vehicles_tmp.semantic_vehicles.insert(
            std::pair<int, common::SemanticVehicle>(semantic_vehicle.vehicle.id(), semantic_vehicle));
    }

    { // 使用swap释放内存(shrink to fit): https://www.cnblogs.com/xiaobingqianrui/p/9092051.html
...
ErrorType SemanticMapManager::OpenloopTrajectoryPrediction() {
    ...
    for (const auto& p_sv : semantic_surrounding_vehicles_.semantic_vehicles) {
        ...
        // pred_time_ = 5.0s, pred_step_ = 0.2s
...
ErrorType SemanticMapManager::UpdateSemanticLaneSet() {
    ...
    // Update semantic lane set
    {
        for (const auto& pe : surrounding_lane_net_.lane_set) { // 循环每条车道线, 使用三次样条拟合采样点
            ...
            if (common::LaneGenerator::GetLaneBySamplePoints(samples, &semantic_lane.lane)
                != kSuccess) { // 使用Spline拟合采样点 (样条的自变量为弧长, 因变量为两轴的坐标X和Y)
                continue;
            }

            semantic_lane_set_.semantic_lanes.insert(
                std::pair<int, common::SemanticLane>(semantic_lane.id, semantic_lane));
        }
    }
    // Check the consistency of semantic map
    {
        ...
        for (auto& semantic_lane : semantic_lane_set_.semantic_lanes) {
            ...
            for (auto it = semantic_lane.second.father_id.begin(); it < semantic_lane.second.father_id.end();) {
                ...
                if (father_it == semantic_lane_set_.semantic_lanes.end()) { // 当前车道线找不到father, 删除
                    it = semantic_lane.second.father_id.erase(it); // erase删除后, 返回下一个迭代器
                } else {
                    ++it;
                }
            }
            for (auto it = semantic_lane.second.child_id.begin(); it < semantic_lane.second.child_id.end();) {
                ...
                if (child_it == semantic_lane_set_.semantic_lanes.end()) { // 当前车道线找不到child, 删除
...
ErrorType SemanticMapManager::UpdateLocalLanesAndFastLut() {
    ...
    int cur_lane_id; // 当前车道线的id
    decimal_t dist_tmp, arc_len_tmp; // ego_state距最近车道线的距离, 最近距离点对应的弧长
    if (GetNearestLaneIdUsingState(ego_state.ToXYTheta(), std::vector<int>(), &cur_lane_id, &dist_tmp, &arc_len_tmp)
        != kSuccess) {
        return kWrongStatus;
    }
    ...
    std::vector<int> root_lane_ids; // cur_id, l_lane_id, ll_lane_id, r_lane_id, rr_lane_id
    ...
    for (const auto root_id : root_lane_ids) { // 遍历cur_id, l_lane_id, ll_lane_id, r_lane_id, rr_lane_id
        ...
        GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(root_id, arc_len, 0.0, std::vector<int>(),
                                                              &all_paths_backward);
        ...
        std::vector<std::vector<int>> assembled_paths; // 所有local path集合
        for (const auto& f_path : all_paths_forward) {
            for (const auto& b_path : all_paths_backward) {
                std::vector<int> path = f_path;
                path.erase(path.begin()); // 删除当前车道线id, 因为b_path.end()已经包含当前车道线id了
                path.insert(path.begin(), b_path.begin(), b_path.end()); // b_path已经reverse过
        ...
        for (const auto& path : assembled_paths) { // 每条path由多个id组成
            common::Lane lane;
            if (kSuccess
                != GetLocalLaneUsingLaneIds(ego_state, path, local_lane_length_forward_, local_lane_length_backward_,
                                            true, &lane)) {
                continue;
            }
            int local_id = local_lane_cnt++;
            local_lanes_.insert(std::pair<int, common::Lane>(local_id, lane)); // local lane id -> Lane

            local_to_segment_lut_.insert(std::pair<int, std::vector<int>>(local_id, path)); // local lane -> ids

            for (const auto& id : path) {
                segment_to_local_lut_[id].insert(local_id); // id -> local lane, 哪些local lane包含了这个id
            }
        }
    }
...
void SemanticMapManager::GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(
    const decimal_t& node_id, const decimal_t& node_length, const decimal_t& aggre_length,
    ...
    // local_lane_length_forward_ = 250.0, Minimum Length, child -> forward
    if (node_aggre_length >= local_lane_length_forward_ || whole_lane_net_.lane_set.at(node_id).child_id.empty()) {
        ...
        for (const auto child_id : child_ids) {
            ...
            GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(child_id, child_length, node_aggre_length, path,
                                                                 all_paths);
        }
    }
}
...
void SemanticMapManager::GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(
    ...
    // local_lane_length_backward_ = 150.0, Minimum Length, father -> backward
    if (node_aggre_length >= local_lane_length_backward_ || whole_lane_net_.lane_set.at(node_id).father_id.empty()) {
        ...
        for (const auto father_id : father_ids) {
            ...
            GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(father_id, father_length, node_aggre_length, path,
                                                                  all_paths);
        }
    }
}
...
ErrorType SemanticMapManager::GetDistanceToLanesUsing3DofState(
    const Vec3f& state, std::set<std::tuple<decimal_t, decimal_t, decimal_t, int>>* res) const {
    for (const auto& p : semantic_lane_set_.semantic_lanes) { // 遍历当前车辆状态附近的每一条车道线
        decimal_t arc_len;
        p.second.lane.GetArcLengthByVecPosition(
            Vec2f(state(0), state(1)), &arc_len); // 求lane上距离给定state的最近点的弧长, Spline的自变量是弧长的近似
        ...
        Vec2f pt;
        p.second.lane.GetPositionByArcLength(arc_len, &pt); // 计算距给定state最近的点
        double dist = std::hypot((state(0) - pt(0)), (state(1) - pt(1))); // 给定state与lane的最近距离

        if (dist > lane_range_) // lane_range_ = 10
            continue;

        decimal_t lane_angle;
        p.second.lane.GetOrientationByArcLength(arc_len, &lane_angle); // 计算arc_len处的一阶导数, 然后求atan
        decimal_t angle_diff = normalize_angle(lane_angle - state(2)); // 车道线的切角和当前车辆状态的夹角的偏差
...
ErrorType SemanticMapManager::CheckCollisionUsingState(const common::VehicleParam& param_a,
...
ErrorType SemanticMapManager::CheckCollisionUsingStateAndVehicleParam(const common::VehicleParam& vehicle_param,
                                                                      const common::State& state, bool* res) {
    ...
    if (agent_config_info_.enable_openloop_prediction) {
        for (const auto& v : semantic_surrounding_vehicles_.semantic_vehicles) {
            ...
            if (access_index < 0 || access_index >= num_pred_states)
                continue;
            bool is_collision = false;
            CheckCollisionUsingState(vehicle_param, state, v.second.vehicle.param(), openloop_pred_traj[access_index],
                                     &is_collision);
            if (is_collision) {
                *res = is_collision;
                return kSuccess;
...
ErrorType SemanticMapManager::IsTopologicallyReachable(const int lane_id, const std::vector<int>& path,
...
ErrorType SemanticMapManager::GetNearestLaneIdUsingState(const Vec3f& state, const std::vector<int>& navi_path, int* id,
                                                         decimal_t* distance, decimal_t* arc_len) const {
    // tuple: dist, arc_len, angle_diff, id :
    // 给定state到lane的最小距离, 最小距离时的弧长, 最小距离处切角和当前车辆状态夹角的偏差, 车道线的id
    std::set<std::tuple<decimal_t, decimal_t, decimal_t, int>> lanes_in_dist;
    if (GetDistanceToLanesUsing3DofState(state, &lanes_in_dist) != kSuccess) { // 获取当前3DofState到lanes的距离(tuple)
        return kWrongStatus;
    }
    ...
    for (const auto& ele : lanes_in_dist) { // lanes_in_dist首先按dist排序, 若dist相同, 再按照arc_len,...
        if (std::get<0>(ele) > nearest_lane_range_) { // 距离阈值, > 1.5m的车道线不考虑
            break;
        }
        // tuple: angle_diff, dist, arc_len, id
        lanes_in_angle_diff.insert(std::tuple<decimal_t, decimal_t, decimal_t, int>(
            fabs(std::get<2>(ele)), std::get<0>(ele), std::get<1>(ele), std::get<3>(ele)));
    }
    // lanes_in_angle_diff.begin()表示angle_diff最小的tuple, 若最小的角度差都大于阈值, 则按照距离选择
    if (lanes_in_angle_diff.empty() || std::get<0>(*lanes_in_angle_diff.begin()) > kPi / 2) {
        // Use the nearest lane with suitable angle diff
        for (const auto& ele : lanes_in_dist) { // 按照dist依次筛选
            if (std::get<2>(ele) < kPi / 2) {
                *id = std::get<3>(ele);
                *distance = std::get<0>(ele); // 元组不支持迭代访问, 且只能通过索引, 且索引不能动态传入
                *arc_len = std::get<1>(ele);
                return kSuccess;
            }
        }
        // Otherwise(all angle_diff is not suitable), use the nearest lane
...
ErrorType SemanticMapManager::TrajectoryPredictionForVehicle(const common::Vehicle& vehicle, const common::Lane& lane,
                                                             const decimal_t& t_pred, const decimal_t& t_step,
                                                             vec_E<common::State>* traj) {
    planning::OnLaneFsPredictor::GetPredictedTrajectory(lane, vehicle, t_pred, t_step, traj);
    return kSuccess;
}
...
ErrorType SemanticMapManager::UpdateKeyVehicles() {
...
#if 1
    decimal_t search_radius = agent_config_info_.surrounding_search_radius; // 150.0

    // key_lane_ids: ego_car可能行驶的前方和后方lanes
    std::map<int, decimal_t> key_lane_ids; // <lane_id, reference arc_len of start points w.r.t to ego vehicle>
    int cur_lane_id;
    decimal_t cur_lane_dist;
    decimal_t cur_arc_len;

    if (GetNearestLaneIdUsingState(ego_vehicle_.state().ToXYTheta(), std::vector<int>(), &cur_lane_id, &cur_lane_dist,
                                   &cur_arc_len)
        != kSuccess) {
        return kWrongStatus;
    }
    ...
    // Find key lane ids
    {
        ...
        if (whole_lane_net_.lane_set.at(cur_lane_id).l_change_avbl) {
            key_lane_ids.insert(
                std::pair<int, decimal_t>(whole_lane_net_.lane_set.at(cur_lane_id).l_lane_id, -cur_arc_len));
        ...
        if (whole_lane_net_.lane_set.at(cur_lane_id).r_change_avbl) {
            key_lane_ids.insert(
                std::pair<int, decimal_t>(whole_lane_net_.lane_set.at(cur_lane_id).r_lane_id, -cur_arc_len));
            mid_lane_ids.push_back(whole_lane_net_.lane_set.at(cur_lane_id).r_lane_id);
        }

        // successors, 向前计算
        decimal_t len_front = whole_lane_net_.lane_set.at(cur_lane_id).length - cur_arc_len; // 剩余距离
        for (const int& id : mid_lane_ids) {
            // ! bug!
            decimal_t len_sum = len_front;
            if (len_sum >= search_radius) // 150.0
                continue;
    ...
    decimal_t t_pl = 5.0;
    decimal_t t_comfort = std::max(t_pl, ego_vehicle_.state().velocity / dec_comfort); // 舒服的减速时间
    decimal_t pl_horizon_length = ego_vehicle_.state().velocity * t_comfort + 100;
    decimal_t front_range = std::min(pl_horizon_length, max_radius); // 规划轨迹的时间长度
    front_range = std::max(min_radius, front_range);

    // get key surrounding vehicles
    // ~ Here we use an assumption:
    // ~ All nearby/reachable lanes have similar length and start from similar arc_len
        ...
        for (const auto v : semantic_surrounding_vehicles_.semantic_vehicles) {
            ...
            if (it != key_lane_ids.end()) {
                ...
                if (dist >= 0 && fabs(dist) < front_range) {
                    ...
                    semantic_key_vehicles_.semantic_vehicles.insert(std::pair<int, common::SemanticVehicle>(
                        v_id, semantic_surrounding_vehicles_.semantic_vehicles.at(v_id)));
                    key_vehicles_.vehicles.insert(std::pair<int, common::Vehicle>(
                        v_id, semantic_surrounding_vehicles_.semantic_vehicles.at(v_id).vehicle));
                }
                // * Rear
...
ErrorType SemanticMapManager::GetLocalLaneSamplesByState(const common::State& state, const int lane_id,
                ...
                if (!father_ids.empty()) {
                    ...
                    for (auto& id : father_ids) {
                        if (std::find(navi_path.begin(), navi_path.end(), id) != navi_path.end()) {
                            father_id = id;
                            break; // find the first father id and break
                        }
                    }
    ...
    decimal_t accum_dist_forward = 0.0;
    {
        ...
        if (dist_remain_target_lane < max_reflane_dist) {
            int id_tmp = lane_id;
            ids_front.push_back(lane_id); // 计算accum_dist_backward时没push, 因为防止cur_id被加入两次
    ...
    SampleLane(long_lane, sample_start,
               sample_start + forward_sample_len + std::min(accum_dist_backward, max_backward_dist), 1.0, samples,
               &acc_dist_tmp);

    return kSuccess;
}

ErrorType SemanticMapManager::GetLocalLaneUsingLaneIds(const common::State& state, const std::vector<int>& lane_ids,
                                                       const decimal_t forward_length, const decimal_t backward_length,
                                                       const bool& is_high_quality, common::Lane* lane) {
    vec_Vecf<2> raw_samples;
    for (const auto& id : lane_ids) { // 将组成一条path的多个连续的车道线整合成一条长的path
    ...
    common::Lane long_lane;
    // 使用三次样条拟合采样点, 生成一条长的lane
    if (common::LaneGenerator::GetLaneBySamplePoints(raw_samples, &long_lane) != kSuccess) {
        return kWrongStatus;
    }
    decimal_t arc_len;
    long_lane.GetArcLengthByVecPosition(state.vec_position, &arc_len); // 给定state, 求对应的车道线上弧长
    ...
    // 将Lane在[start, end]上以step = 1.0进行采样
    SampleLane(long_lane, sample_start, sample_end, 1.0, &samples, &acc_dist_tmp);

    if (kSuccess != GetLaneBySampledPoints(samples, is_high_quality, lane)) { // is_high_quality = true
        return kWrongStatus;
    }
...
ErrorType SemanticMapManager::GetRefLaneForStateByBehavior(const common::State& state,
                                                           const std::vector<int>& navi_path,
    ...
    if (distance_to_lane > max_distance_to_lane_) { // max_distance_to_lane_ = 2.0
        return kWrongStatus;
    }

    int target_lane_id;
    if (GetTargetLaneId(current_lane_id, behavior, &target_lane_id) != kSuccess) {
        // printf("[GetRefLaneForStateByBehavior]fail to get target lane from lane %d "
        //        "with behavior %d.\n",
    ...
    if (agent_config_info_.enable_fast_lane_lut && has_fast_lut_) {
        if (segment_to_local_lut_.end() != segment_to_local_lut_.find(target_lane_id)) {
            ...
            int id = *segment_to_local_lut_.at(target_lane_id).begin(); // find lut local lane
    ...
    vec_Vecf<2> samples;
    if (GetLocalLaneSamplesByState(state, target_lane_id, navi_path, max_forward_len, max_back_len, &samples)
        != kSuccess) {
        printf("[GetRefLaneForStateByBehavior]Cannot get local lane samples.\n");
        return kWrongStatus;
    }

    if (kSuccess != GetLaneBySampledPoints(samples, is_high_quality, lane)) { // is_high_quality = false
        return kWrongStatus;
    }
...
ErrorType SemanticMapManager::GetLaneBySampledPoints(const vec_Vecf<2>& samples, const bool& is_high_quality,
                                                     common::Lane* lane) const {
    if (is_high_quality) {
        double d = 0.0;
        ...
        // LinSpaced: 线性分布的数组, VectorXd::LinSpaced(size,low,high)
...
ErrorType SemanticMapManager::SampleLane(const common::Lane& lane, const decimal_t& s0, const decimal_t& s1,
                                         const decimal_t& step, vec_E<Vecf<2>>* samples, decimal_t* accum_dist) const {
    Vecf<2> pt;
...
ErrorType SemanticMapManager::GetTargetLaneId(const int lane_id, const LateralBehavior& behavior,
...
ErrorType SemanticMapManager::GetLeadingVehicleOnLane(const common::Lane& ref_lane, const common::State& ref_state,
    ...
    for (decimal_t s = ref_fs.vec_s[0] + resolution + search_lon_offset;
         s < ref_fs.vec_s[0] + max_forward_search_dist + search_lon_offset; s += resolution) {
...
ErrorType SemanticMapManager::GetFollowingVehicleOnLane(const common::Lane& ref_lane, const common::State& ref_state,
    ...
    for (decimal_t delta_s = lane_width_tol / 1.4; delta_s < max_backward_search_dist - 2.0 * lane_width_tol;
...
ErrorType SemanticMapManager::GetTrafficStoppingState(const State& state, const Lane& lane,
...
ErrorType SemanticMapManager::GetDistanceOnLaneNet(const int& lane_id_0, const decimal_t& arc_len_0,
                                                   const int& lane_id_1, const decimal_t& arc_len_1,
