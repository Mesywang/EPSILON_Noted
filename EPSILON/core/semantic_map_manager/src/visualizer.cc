...
Visualizer::Visualizer(ros::NodeHandle nh, int node_id) : nh_(nh), node_id_(node_id) {
    ...
    std::string ego_vehicle_vis_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/ego_vehicle_vis");
    std::string obstacle_map_vis_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/obstacle_map");
    std::string surrounding_lane_net_vis_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/surrounding_lane_net_vis");
    std::string local_lanes_vis_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/local_lanes_vis");
    std::string ego_vehicle_behavior_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/ego_behavior_vis");
    std::string pred_intention_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/pred_initial_intention_vis");
    std::string pred_traj_openloop_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/pred_traj_openloop_vis");
    std::string surrounding_vehicle_topic =
        std::string("/vis/agent_") + std::to_string(node_id_) + std::string("/surrounding_vehicle_vis");
...
void Visualizer::VisualizeDataWithStampForPlayback(const ros::Time& stamp, const SemanticMapManager& smm,
...
void Visualizer::VisualizeSurroundingVehicles(const ros::Time& stamp, const common::VehicleSet& vehicle_set,
    ...
    for (const auto& v : vehicle_set.vehicles) {
        ...
        if (nearby_ids.end() != std::find(nearby_ids.begin(), nearby_ids.end(), v.second.id())) {
            color_obb.a = 0.9;
            color_vel_vec.a = 0.9;
            color_steer.a = 0.9;
        }
        common::VisualizationUtil::GetRosMarkerArrayUsingVehicle(v.second, color_obb, color_vel_vec, color_steer, 1,
                                                                 &vehicle_marker);
    ...
    common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("/map"),
                                                         last_surrounding_vehicle_marker_cnt_, &vehicle_marker_list);
...
void Visualizer::VisualizeEgoVehicle(const ros::Time& stamp, const common::Vehicle& vehicle) {
    ...
    common::VisualizationUtil::GetRosMarkerArrayUsingVehicle(vehicle, color_obb, color_vel_vec, color_steer, 1,
                                                             &vehicle_marker);
    common::VisualizationUtil::FillStampInMarkerArray(stamp, &vehicle_marker);
    ego_vehicle_pub_.publish(vehicle_marker);
}

void Visualizer::VisualizeObstacleMap(const ros::Time& stamp,
...
void Visualizer::VisualizeSurroundingLaneNet(const ros::Time& stamp, const common::LaneNet& lane_net,
                                             const std::vector<int>& deleted_lane_ids) {
    ...
    for (auto iter = lane_net.lane_set.begin(); iter != lane_net.lane_set.end(); ++iter) {
        ...
        common::VisualizationUtil::GetRosMarkerLineStripUsing2DofVec(
            iter->second.lane_points, common::cmap.at("sky blue"), Vec3f(0.1, 0.1, 0.1), iter->second.id, &lane_marker);
        ...
        visualization_msgs::Marker start_point_marker, end_point_marker, lane_id_text_marker;
        {
            ...
            common::VisualizationUtil::GetRosMarkerSphereUsingPoint(
                Vec3f(pt(0), pt(1), 0.0), common::ColorARGB(1.0, 0.2, 0.6, 1.0), Vec3f(0.5, 0.5, 0.5), id_cnt++,
                &start_point_marker);
            ...
            common::VisualizationUtil::GetRosMarkerTextUsingPositionAndString(
                Vec3f(pt(0), pt(1), 0.5), std::to_string(iter->second.id), common::ColorARGB(1.0, 0.0, 0.0, 1.0),
                Vec3f(0.6, 0.6, 0.6), id_cnt++, &lane_id_text_marker);
        }
        {
            end_point_marker.header.stamp = stamp;
            end_point_marker.header.frame_id = "map";
            Vec2f pt = *(iter->second.lane_points.rbegin());
            common::VisualizationUtil::GetRosMarkerSphereUsingPoint(Vec3f(pt(0), pt(1), 0.0),
                                                                    common::ColorARGB(1.0, 0.2, 0.6, 1.0),
    ...
    common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("/map"), last_surrounding_lanes_cnt_,
                                                         &lane_net_marker);
    last_surrounding_lanes_cnt_ = num_markers;
    surrounding_lane_net_pub_.publish(lane_net_marker);
}

void Visualizer::VisualizeLocalLanes(const ros::Time& stamp, const std::unordered_map<int, common::Lane>& local_lanes,
    ...
    for (const auto& p_lane : local_lanes) {
        ...
        common::VisualizationUtil::GetMarkerByLane(p_lane.second, 1.0, Vec3f(1.0, 0.0, 0.0),
...
void Visualizer::VisualizeBehavior(const ros::Time& stamp, const common::SemanticBehavior& behavior) {
    ...
    common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("/map"), last_behavior_marker_cnt_,
                                                         &behavior_marker_arr);
...
void Visualizer::VisualizeSpeedLimit(const ros::Time& stamp, const vec_E<common::SpeedLimit>& speed_limits) {
    ...
    for (int i = 0; i < static_cast<int>(speed_limits.size()); ++i) {
        ...
        common::VisualizationUtil::GetRosMarkerMeshHexagonSignUsingPosition(
            Vec3f(start_x, start_y, start_point_angle), 1.0, start_marker_color, ++id_cnt, &start_marker);
        ...
        str_start +=
            std::string(common::GetStringByValueWithPrecision<decimal_t>(speed_limits[i].vel_range()(1), 1) + "m/s");
        common::VisualizationUtil::GetRosMarkerTextUsingPositionAndString(
            Vec3f(start_x, start_y, 3.5), str_start, common::cmap.at("black"), Vec3f(0.75, 0.75, 0.75), ++id_cnt,
            &start_text_marker);
        ...
        common::VisualizationUtil::GetRosMarkerMeshHexagonSignUsingPosition(
            Vec3f(end_x, end_y, speed_limits[i].end_angle()), 1.0, end_marker_color, ++id_cnt, &end_marker);
        ...
        common::VisualizationUtil::GetRosMarkerTextUsingPositionAndString(
            Vec3f(end_x, end_y, 3.5), str_end, common::cmap.at("black"), Vec3f(0.75, 0.75, 0.75), ++id_cnt,
            &end_text_marker);
        traffic_signal_arr.markers.push_back(end_text_marker);
    }
    int num_markers = static_cast<int>(traffic_signal_arr.markers.size());
    common::VisualizationUtil::FillHeaderIdInMarkerArray(stamp, std::string("/map"), last_speed_limit_marker_cnt_,
                                                         &traffic_signal_arr);
    last_speed_limit_marker_cnt_ = num_markers;
    speed_limit_vis_pub_.publish(traffic_signal_arr);
}

void Visualizer::VisualizeIntentionPrediction(const ros::Time& stamp,
...
void Visualizer::VisualizeOpenloopTrajPrediction(
    const ros::Time& stamp, const std::unordered_map<int, vec_E<common::State>>& openloop_pred_trajs) {
    visualization_msgs::MarkerArray mks;
    ...
    for (const auto& p_traj : openloop_pred_trajs) {
        ...
        common::VisualizationUtil::GetRosMarkerLineStripUsingPoints(points, Vec3f(0.1, 0.1, 0.1),
                                                                    common::cmap.at("sky blue"), 0, &line_marker);
