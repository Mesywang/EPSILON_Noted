...
public:
    ...
    static ErrorType GetTargetStateOnTargetLane(const common::StateTransformer& stf_target,
        ...
        if (gap_front_vehicle.id() != -1
            ...
            has_front = true;
            s_ref_front =
                front_fs.vec_s[0] - (gap_front_vehicle.param().length() / 2.0 - gap_front_vehicle.param().d_cr());
            s_thres_front = s_ref_front - min_spacing - time_headaway * ego_vehicle.state().velocity;
        }
        ...
        if (gap_rear_vehicle.id() != -1
            ...
            has_rear = true;
            s_ref_rear = rear_fs.vec_s[0] + gap_rear_vehicle.param().length() / 2.0 + gap_rear_vehicle.param().d_cr();
            s_thres_rear = s_ref_rear + min_spacing + time_headaway * gap_rear_vehicle.state().velocity;
        }
        ...
        decimal_t ego_desired_vel =
            ego_vehicle.state().velocity + (param.idm_param.kDesiredVelocity - ego_vehicle.state().velocity) * p_v_ego;
        if (has_front && has_rear) {
            ...
            decimal_t v_ref_front =
                std::max(0.0, gap_front_vehicle.state().velocity + truncate(s_err_front * k_v, dv_lb, dv_ub));

            decimal_t s_err_rear = s_thres_rear - ego_fs.vec_s[0];
            decimal_t v_ref_rear =
                std::max(0.0, gap_rear_vehicle.state().velocity + truncate(s_err_rear * k_v, dv_lb, dv_ub));
        ...
        target_fs.Load(Vecf<3>(desired_s, desired_v, 0.0), Vecf<3>(0.0, 0.0, 0.0), common::FrenetState::kInitWithDs);
    ...
    static ErrorType PropagateOnceAdvancedLK(const common::StateTransformer& stf, const common::Vehicle& ego_vehicle,
        ...
        if (!steer_calculation_failed) {
            decimal_t approx_lookahead_dist = std::min(
                std::max(param.steer_control_min_lookahead_dist, current_state.velocity * param.steer_control_gain),
                param.steer_control_max_lookahead_dist);
            if (CalcualateSteer(stf, current_state, current_fs, wheelbase_len,
                                Vec2f(approx_lookahead_dist, lat_track_offset), &steer)
                != kSuccess) {
                steer_calculation_failed = true;
            }
        }
        ...
        if (leading_vehicle.id() == kInvalidAgentId
            || stf.GetFrenetStateFromState(leading_vehicle.state(), &leading_fs) != kSuccess) {
            // ~ Without leading vehicle
            CalcualateVelocityUsingIdm(current_state.velocity, dt, sim_param, &velocity);
        } else {
            ...
            CalcualateVelocityUsingIdm(current_fs.vec_s[0], current_state.velocity, leading_fs.vec_s[0],
    ...
    static ErrorType PropagateOnceAdvancedLC(const common::StateTransformer& stf_current,
                                             const common::StateTransformer& stf_target,
                                             const common::Vehicle& ego_vehicle, const Vehicle& current_leading_vehicle,
                                             const Vehicle& gap_front_vehicle, const Vehicle& gap_rear_vehicle,
                                             const decimal_t& lat_track_offset, const decimal_t& dt, const Param& param,
                                             State* desired_state) {
        common::State current_state = ego_vehicle.state();
        decimal_t wheelbase_len = ego_vehicle.param().wheel_base();
        auto sim_param = param;
        ...
        if (stf_target.GetFrenetStateFromState(current_state, &ego_on_tarlane_fs) != kSuccess
        ...
        if (!steer_calculation_failed) {
            decimal_t approx_lookahead_dist = std::min(
                std::max(param.steer_control_min_lookahead_dist, current_state.velocity * param.steer_control_gain),
                param.steer_control_max_lookahead_dist);
            if (CalcualateSteer(stf_target, current_state, ego_on_tarlane_fs, wheelbase_len,
                                Vec2f(approx_lookahead_dist, lat_track_offset), &steer)
                != kSuccess) {
                steer_calculation_failed = true;
            }
        ...
        common::State target_state;
        if (kSuccess
            != GetTargetStateOnTargetLane(stf_target, ego_vehicle, gap_front_vehicle, gap_rear_vehicle, sim_param,
                                          &target_state)) {
        ...
        if (current_leading_vehicle.id() == kInvalidAgentId
            || stf_current.GetFrenetStateFromState(current_leading_vehicle.state(), &current_leading_fs) != kSuccess) {
            // ~ Without leading vehicle
            CalcualateVelocityUsingCtxIdm(ego_on_tarlane_fs.vec_s[0], current_state.velocity,
            ...
            // * between ego vehicle and the leading vehicle.
            decimal_t eqv_vehicle_len;
            GetIdmEquivalentVehicleLength(stf_current, ego_vehicle, current_leading_vehicle, current_leading_fs,
                                          &eqv_vehicle_len);
            sim_param.idm_param.kVehicleLength = eqv_vehicle_len;
    ...
    static ErrorType PropagateOnce(const common::StateTransformer& stf, const common::Vehicle& ego_vehicle,
                                   const Vehicle& leading_vehicle, const decimal_t& dt, const Param& param,
                                   State* desired_state) {
        common::State current_state = ego_vehicle.state();
        decimal_t wheelbase_len = ego_vehicle.param().wheel_base(); // 轴距: 前后轮中心之间的距离
        auto sim_param = param;

        // * 假设该车辆会跟随车道线, 基于此预测其行驶轨迹, 即使用geometric controller前向积分
        ...
        decimal_t steer, velocity;
        if (!steer_calculation_failed) {
            // steer_control_min_lookahead_dist = 3.0, steer_control_gain = 1.5, steer_control_max_lookahead_dist = 50.0
            decimal_t approx_lookahead_dist = std::min(
                std::max(param.steer_control_min_lookahead_dist, current_state.velocity * param.steer_control_gain),
                param.steer_control_max_lookahead_dist);
            // PurePursuit计算转角steer
            if (CalcualateSteer(stf, current_state, current_fs, wheelbase_len, Vec2f(approx_lookahead_dist, 0.0),
                                &steer)
                != kSuccess) {
                steer_calculation_failed = true;
            }
        ...
        steer = steer_calculation_failed ? current_state.steer : steer; // 若failed保持当前steer
        decimal_t sim_vel = param.idm_param.kDesiredVelocity;
        // auto_decelerate_if_lat_failed = true: 若steer_calculation_failed, 自动减速
        if (param.auto_decelerate_if_lat_failed && steer_calculation_failed) {
            sim_vel = 0.0;
        }
        sim_param.idm_param.kDesiredVelocity = std::max(0.0, sim_vel);
        ...
        if (leading_vehicle.id() == kInvalidAgentId
            || stf.GetFrenetStateFromState(leading_vehicle.state(), &leading_fs) != kSuccess) {
            // ~ Without leading vehicle
            CalcualateVelocityUsingIdm(current_state.velocity, dt, sim_param, &velocity); // TODO: Idm to be read
        } else {
            // ~ With leading vehicle
            // * For IDM, vehicle length is subtracted to get the 'net' distance
            // * between ego vehicle and the leading vehicle.
        ...
        // * Step III: Get desired state using vehicle kinematic model
        // TODO: vehicle kinematic model to be read
        CalculateDesiredState(current_state, steer, velocity, wheelbase_len, dt, sim_param, desired_state);
        return kSuccess;
    }

    // ~ Propagate using const velocity and steer
    static ErrorType PropagateOnce(const decimal_t& desired_vel, const common::Vehicle& ego_vehicle,
                                   const decimal_t& dt, const Param& param, State* desired_state) {
        common::State current_state = ego_vehicle.state();
        decimal_t wheelbase_len = ego_vehicle.param().wheel_base();
        decimal_t steer = current_state.steer;
        decimal_t velocity = current_state.velocity;
        CalculateDesiredState(current_state, steer, velocity, wheelbase_len, dt, param, desired_state);
        return kSuccess;
    }

private:
    static ErrorType GetIdmEquivalentVehicleLength(const common::StateTransformer& stf,
                                                   const common::Vehicle& ego_vehicle,
        ...
        *eqv_vehicle_len = ego_vehicle.param().length() / 2.0 + ego_vehicle.param().d_cr() + len_rb2r;

        return kSuccess;
    }

    static ErrorType CalcualateSteer(const common::StateTransformer& stf, const State& current_state,
                                     const FrenetState& current_fs, const decimal_t& wheelbase_len,
                                     const Vec2f& lookahead_offset, decimal_t* steer) {
        common::FrenetState dest_fs; // fs系下的lookahead point
        dest_fs.Load(Vecf<3>(lookahead_offset(0) + current_fs.vec_s[0], 0.0, 0.0),
                     Vecf<3>(lookahead_offset(1), 0.0, 0.0), common::FrenetState::kInitWithDs);
        ...
        decimal_t look_ahead_dist = (dest_state.vec_position - current_state.vec_position).norm(); // ld
        decimal_t cur_to_dest_angle = vec2d_to_angle(dest_state.vec_position - current_state.vec_position);
        decimal_t angle_diff = normalize_angle(cur_to_dest_angle - current_state.angle); // α
        // PurePursuit参数: L, α, ld, δ
        control::PurePursuitControl::CalculateDesiredSteer(wheelbase_len, angle_diff, look_ahead_dist, steer); 
        return kSuccess;
    }

    // ~ Using leading vehicle
    static ErrorType CalcualateVelocityUsingIdm(const decimal_t& current_pos, const decimal_t& current_vel,
        ...
        return control::IntelligentVelocityControl::CalculateDesiredVelocity(
            param.idm_param, current_pos, leading_pos, current_vel, leading_vel_fin, dt, velocity);
    }

    // ~ Using virtual leading vehicle
    static ErrorType CalcualateVelocityUsingIdm(const decimal_t& current_vel, const decimal_t& dt, const Param& param,
                                                decimal_t* velocity) {
        const decimal_t virtual_leading_dist = 100.0 + 100.0 * current_vel;
        return control::IntelligentVelocityControl::CalculateDesiredVelocity(
            param.idm_param, 0.0, 0.0 + virtual_leading_dist, current_vel, current_vel, dt, velocity);
    }

    static ErrorType CalcualateVelocityUsingCtxIdm(const decimal_t& current_pos, const decimal_t& current_vel,
                                                   const decimal_t& leading_pos, const decimal_t& leading_vel,
        ...
        return control::ContextIntelligentVelocityControl::CalculateDesiredVelocity(
            param.idm_param, ctx_param, current_pos, leading_pos, target_pos, current_vel, leading_vel_fin, target_vel,
            dt, velocity);
    }
    ...
    static ErrorType CalcualateVelocityUsingCtxIdm(const decimal_t& current_pos, const decimal_t& current_vel,
                                                   ...
                                                   const decimal_t& dt, const Param& param, const CtxParam& ctx_param,
                                                   decimal_t* velocity) {
        const decimal_t virtual_leading_pos = current_pos + 100.0 + 100.0 * current_vel;
        return control::ContextIntelligentVelocityControl::CalculateDesiredVelocity(
            param.idm_param, ctx_param, current_pos, virtual_leading_pos, target_pos, current_vel, current_vel,
            target_vel, dt, velocity);
    }

    static ErrorType CalculateDesiredState(const State& current_state, const decimal_t steer, const decimal_t velocity,
                                           const decimal_t wheelbase_len, const decimal_t dt, const Param& param,
                                           State* state) {
        simulator::IdealSteerModel model(
            wheelbase_len, param.idm_param.kAcceleration, param.idm_param.kHardBrakingDeceleration,
            param.max_lon_acc_jerk, param.max_lon_brake_jerk, param.max_lat_acceleration_abs, param.max_lat_jerk_abs,
            param.max_steer_angle_abs, param.max_steer_rate, param.max_curvature_abs);
