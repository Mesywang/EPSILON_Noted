...
ErrorType BehaviorPlanner::RunMpdm() {
    ...
    // behavior_.actual_desired_velocity = 5.0
    if (MultiBehaviorJudge(behavior_.actual_desired_velocity, &mpdm_behavior, &mpdm_desired_velocity) == kSuccess) {
...
ErrorType BehaviorPlanner::RunRoutePlanner(const int nearest_lane_id) {
    ...
    if (!p_route_planner_->if_get_lane_net()) { // 只获得一次WholeLaneNet
...
ErrorType BehaviorPlanner::RunOnce() {
    ...
    int ego_lane_id_by_pos = kInvalidLaneId; // nearest lane
    if (map_itf_->GetEgoLaneIdByPosition(p_route_planner_->navi_path(), &ego_lane_id_by_pos) != kSuccess) {
        printf("[BP RunOnce]Err - Ego not on lane.\n");
        return kWrongStatus;
    }
    // map_itf_是map_adapter_的父类, 调用虚函数
    ...
    if (ego_lane_id_ == kInvalidAgentId) { // 只有Invalid时才更新ego_lane_id
        UpdateEgoLaneId(ego_lane_id_by_pos);
    }

    LateralBehavior behavior_by_lane_id;
    if (JudgeBehaviorByLaneId(ego_lane_id_by_pos, &behavior_by_lane_id) != kSuccess) {
        printf("[RunOnce]fail to judge behavior by lane id!\n");
        return kWrongStatus;
    }

    UpdateEgoLaneId(ego_lane_id_by_pos); // 更新ego_lane_id
    ...
    behavior_.actual_desired_velocity = user_desired_velocity_; // 5.0
    ...
    if (ConstructReferenceLane(behavior_.lat_behavior, &behavior_.ref_lane) != kSuccess) {
        return kWrongStatus;
    }
    return kSuccess;
}

ErrorType BehaviorPlanner::MultiBehaviorJudge(const decimal_t previous_desired_vel, LateralBehavior* mpdm_behavior,
                                              decimal_t* mpdm_desired_velocity) {
    ...
    std::cout << "[BehaviorPlanner]" << ego_vehicle.id()
              << "\tsemantic_vehicle_set num:" << semantic_vehicle_set.semantic_vehicles.size() << std::endl;
    ...
    std::vector<LateralBehavior> potential_behaviors{common::LateralBehavior::kLaneKeeping};
    ...
    for (auto it = semantic_vehicle_set.semantic_vehicles.begin(); it != semantic_vehicle_set.semantic_vehicles.end();
         ++it) { // 遍历每一辆Key Surrounding Car
        common::LateralBehavior lat_behavior;
        if (map_itf_->GetPredictedBehavior(it->second.vehicle.id(), &lat_behavior) != kSuccess) {
            lat_behavior = common::LateralBehavior::kLaneKeeping;
        }
        decimal_t forward_lane_len = std::max(it->second.vehicle.state().velocity * 10.0, 50.0);
        common::Lane ref_lane;
        if (map_itf_->GetRefLaneForStateByBehavior(it->second.vehicle.state(), std::vector<int>(), lat_behavior,
                                                   forward_lane_len, max_backward_len, false, &ref_lane)
            == kSuccess) {
    ...
    for (int i = 0; i < num_available_behaviors; i++) {
        ...
        if (SimulateEgoBehavior(ego_vehicle, potential_behaviors[i], semantic_vehicle_set, &traj, &sur_trajs)
            != kSuccess) {
    ...
    if (EvaluateMultiPolicyTrajs(valid_behaviors, valid_forward_trajs, valid_surround_trajs, &winner_behavior,
                                 &winner_forward_traj, &winner_score, &winner_desired_vel)
        != kSuccess) {
    ...
    if (fabs(winner_desired_vel - ego_vehicle.state().velocity) > max_vel_cmd_gap) {
    ...
    if (lock_to_hmi_) {
        auto it = std::find(forward_behaviors_.begin(), forward_behaviors_.end(), hmi_behavior_);
...
ErrorType BehaviorPlanner::OpenloopSimForward(const common::SemanticVehicle& ego_semantic_vehicle,
    ...
    for (int i = 0; i < num_steps_forward; i++) {
        ...
        if (planning::OnLaneForwardSimulation::PropagateOnce(common::StateTransformer(ego_semantic_vehicle.lane),
                                                             cur_ego_vehicle, common::Vehicle(), sim_resolution_,
                                                             sim_param_, &ego_state)
            != kSuccess) {
            return kWrongStatus;
        }
        ...
        for (auto& v : semantic_vehicle_set_tmp.semantic_vehicles) {
            ...
            if (planning::OnLaneForwardSimulation::PropagateOnce(common::StateTransformer(v.second.lane),
                                                                 v.second.vehicle, common::Vehicle(), sim_resolution_,
                                                                 sim_param_, &agent_state)
                != kSuccess) {
                return kWrongStatus;
            }
...
ErrorType BehaviorPlanner::SimulateEgoBehavior(const common::Vehicle& ego_vehicle, const LateralBehavior& ego_behavior,
    ...
    if (map_itf_->GetRefLaneForStateByBehavior(ego_vehicle.state(), p_route_planner_->navi_path(), ego_behavior,
                                               forward_lane_len, max_backward_len, false, &ego_reflane)
        != kSuccess) {
    ...
    semantic_vehicle_set_tmp.semantic_vehicles.insert(std::make_pair(ego_vehicle.id(), ego_semantic_vehicle));
...
ErrorType BehaviorPlanner::EvaluateMultiPolicyTrajs(
    const std::vector<LateralBehavior>& valid_behaviors, const vec_E<vec_E<common::Vehicle>>& valid_forward_trajs,
    const vec_E<std::unordered_map<int, vec_E<common::Vehicle>>>& valid_surround_trajs,
    LateralBehavior* winner_behavior, vec_E<common::Vehicle>* winner_forward_traj, decimal_t* winner_score,
    ...
    for (int i = 0; i < num_valid_behaviors; i++) {
        decimal_t score, vel;
        EvaluateSinglePolicyTraj(valid_behaviors[i], valid_forward_trajs[i], valid_surround_trajs[i], &score, &vel);
...
ErrorType BehaviorPlanner::EvaluateSafetyCost(const vec_E<common::Vehicle>& traj_a,
                                              const vec_E<common::Vehicle>& traj_b, decimal_t* cost) {
    ...
    for (int i = 0; i < num_states; i++) {
        ...
        common::SemanticsUtils::InflateVehicleBySize(traj_a[i], 1.0, 1.0, &inflated_a);
        common::SemanticsUtils::InflateVehicleBySize(traj_b[i], 1.0, 1.0, &inflated_b);
        bool is_collision = false;
        map_itf_->CheckCollisionUsingState(inflated_a.param(), inflated_a.state(), inflated_b.param(),
                                           inflated_b.state(), &is_collision);
        if (is_collision) {
            cost_tmp += 0.01 * fabs(traj_a[i].state().velocity - traj_b[i].state().velocity) * 0.5;
        }
    }
    *cost = cost_tmp;
    return kSuccess;
...
ErrorType
BehaviorPlanner::EvaluateSinglePolicyTraj(const LateralBehavior& behavior, const vec_E<common::Vehicle>& forward_traj,
    ...
    decimal_t cost_efficiency_ego_to_desired_vel =
        fabs(ego_vehicle_terminal.state().velocity - reference_desired_velocity_) / 10.0;
    common::Vehicle leading_vehicle;
    common::Lane ego_ref_lane;
    const decimal_t max_backward_len = 10.0;
    decimal_t forward_lane_len = std::max(ego_vehicle_terminal.state().velocity * 10.0, 50.0);
    if (map_itf_->GetRefLaneForStateByBehavior(ego_vehicle_terminal.state(), p_route_planner_->navi_path(),
                                               common::LateralBehavior::kLaneKeeping, forward_lane_len,
                                               max_backward_len, false, &ego_ref_lane)
        != kSuccess) {
    ...
    if (map_itf_->GetLeadingVehicleOnLane(ego_ref_lane, ego_vehicle_terminal.state(), vehicle_set, lat_range,
                                          &leading_vehicle, &distance_residual_ratio)
        == kSuccess) {
        decimal_t distance_to_leading_vehicle =
            (leading_vehicle.state().vec_position - ego_vehicle_terminal.state().vec_position).norm();

        if (ego_vehicle_terminal.state().velocity < reference_desired_velocity_
            && leading_vehicle.state().velocity < reference_desired_velocity_ && distance_to_leading_vehicle < 100.0) {
            cost_efficiency_leading_to_desired_vel =
                1.5 * distance_residual_ratio
                * fabs(ego_vehicle_terminal.state().velocity - reference_desired_velocity_)
                / std::max(2.0, distance_to_leading_vehicle);
        }
    }
    decimal_t cost_efficiency = 0.5 * (cost_efficiency_ego_to_desired_vel + cost_efficiency_leading_to_desired_vel);
    ...
    *score = cost_action + cost_safety + cost_efficiency;
    printf("[CostDebug]behaivor %d: (action %lf, safety %lf, efficiency ego %lf, "
           "leading %lf).\n",
           static_cast<int>(behavior), cost_action, cost_safety, cost_efficiency_ego_to_desired_vel,
           cost_efficiency_leading_to_desired_vel);
    GetDesiredVelocityOfTrajectory(forward_traj, desired_vel);
    return kSuccess;
}

ErrorType BehaviorPlanner::GetDesiredVelocityOfTrajectory(const vec_E<common::Vehicle> vehicle_vec, decimal_t* vel) {
...
ErrorType BehaviorPlanner::MultiAgentSimForward(const int ego_id,
                                                const common::SemanticVehicleSet& semantic_vehicle_set,
                                                vec_E<common::Vehicle>* traj,
                                                std::unordered_map<int, vec_E<common::Vehicle>>* surround_trajs) {
    traj->clear();
    ...
    for (int i = 0; i < num_steps_forward; i++) {
        ...
        std::unordered_map<int, State> state_cache;
        for (auto& v : semantic_vehicle_set_tmp.semantic_vehicles) {
            decimal_t desired_vel = semantic_vehicle_set.semantic_vehicles.at(v.first).vehicle.state().velocity;
            decimal_t init_stamp = semantic_vehicle_set.semantic_vehicles.at(v.first).vehicle.state().time_stamp;
            if (v.first == ego_id)
            ...
            for (auto& v_other : semantic_vehicle_set_tmp.semantic_vehicles) {
                ...
                if (v_other.first != v.first)
                    vehicle_set.vehicles.insert(std::make_pair(v_other.first, v_other.second.vehicle));
            }

            decimal_t speed_limit;
            if (map_itf_->GetSpeedLimit(v.second.vehicle.state(), v.second.lane, &speed_limit) == kSuccess) {
            ...
            decimal_t distance_residual_ratio = 0.0;
            const decimal_t lat_range = 2.2;
            if (map_itf_->GetLeadingVehicleOnLane(v.second.lane, v.second.vehicle.state(), vehicle_set, lat_range,
                                                  &leading_vehicle, &distance_residual_ratio)
                == kSuccess) {
                bool is_collision = false;
                map_itf_->CheckCollisionUsingState(v.second.vehicle.param(), v.second.vehicle.state(),
            ...
            if (planning::OnLaneForwardSimulation::PropagateOnce(common::StateTransformer(v.second.lane),
                                                                 v.second.vehicle, leading_vehicle, sim_resolution_,
                                                                 sim_param_, &state)
                != kSuccess) {
        ...
        for (auto& s : state_cache) {
            semantic_vehicle_set_tmp.semantic_vehicles.at(s.first).vehicle.set_state(s.second);
            if (s.first == ego_id) {
                traj->push_back(semantic_vehicle_set_tmp.semantic_vehicles.at(s.first).vehicle);
            } else {
                surround_trajs->at(s.first).push_back(semantic_vehicle_set_tmp.semantic_vehicles.at(s.first).vehicle);
            }
        }
...
ErrorType BehaviorPlanner::ConstructReferenceLane(const LateralBehavior& lat_behavior, Lane* lane) {
    if (map_itf_ == nullptr)
        return kWrongStatus;

    // * get target_lane_id
    int target_lane_id;
    if (lat_behavior == common::LateralBehavior::kLaneKeeping || lat_behavior == common::LateralBehavior::kUndefined) {
        ...
        if (map_itf_->GetLeftLaneId(ego_lane_id_, &target_lane_id) != kSuccess) {
            ...
            target_lane_id = ego_lane_id_;
            behavior_.lat_behavior = common::LateralBehavior::kLaneKeeping; // 更换target_lane后直接Keeping新的目标即可
        }
    ...
    // * construct lane
    const decimal_t kMaxReflaneDist = 150.0; // 车道线的前向长度(弧长)
    const decimal_t kBackwardDist = 20.0; // 车道线的后向长度(弧长)
    vec_Vecf<2> samples;
    if (map_itf_->GetLocalLaneSamplesByState(ego_state, target_lane_id, p_route_planner_->navi_path(), kMaxReflaneDist,
                                             kBackwardDist, &samples)
        != kSuccess) {
        return kWrongStatus;
    }
    ...
    for (decimal_t s = current_fs.vec_s[0]; s < current_fs.vec_s[0] + s_forward; s += resolution) {
...
ErrorType BehaviorPlanner::ConstructLaneFromSamples(const vec_E<Vecf<2>>& samples, Lane* lane) {
    double d = 0.0;
...
ErrorType BehaviorPlanner::JudgeBehaviorByLaneId(const int ego_lane_id_by_pos, LateralBehavior* behavior_by_lane_id) {
    if (ego_lane_id_by_pos == ego_lane_id_) { // 还在当前车道线上
        *behavior_by_lane_id = common::LateralBehavior::kLaneKeeping;
        return kSuccess;
    }

    auto it = std::find(potential_lk_lane_ids_.begin(), potential_lk_lane_ids_.end(), ego_lane_id_by_pos);
    auto it_lcl = std::find(potential_lcl_lane_ids_.begin(), potential_lcl_lane_ids_.end(), ego_lane_id_by_pos);
    auto it_lcr = std::find(potential_lcr_lane_ids_.begin(), potential_lcr_lane_ids_.end(), ego_lane_id_by_pos);
...
ErrorType BehaviorPlanner::UpdateEgoBehavior(const LateralBehavior& behavior_by_lane_id) {
    ...
    } else if (behavior_.lat_behavior == common::LateralBehavior::kLaneChangeLeft) {
        if (behavior_by_lane_id == common::LateralBehavior::kLaneKeeping) {
            // ~ still in the course of lane changing
        } else if (behavior_by_lane_id == common::LateralBehavior::kLaneChangeLeft) {
    ...
    } else if (behavior_.lat_behavior == common::LateralBehavior::kLaneChangeRight) {
        if (behavior_by_lane_id == common::LateralBehavior::kLaneKeeping) {
            // ~ still in the course of lane changing
        } else if (behavior_by_lane_id == common::LateralBehavior::kLaneChangeRight) {
            // ~ lane change complete
            behavior_.lat_behavior = common::LateralBehavior::kLaneKeeping;
            lock_to_hmi_ = false;
        } else if (behavior_by_lane_id == common::LateralBehavior::kUndefined) {
...
ErrorType BehaviorPlanner::UpdateEgoLaneId(const int new_ego_lane_id) {
    ...
    // 查找ego_car可能行驶的lanes
    GetPotentialLaneIds(ego_lane_id_, common::LateralBehavior::kLaneKeeping, &potential_lk_lane_ids_);
    GetPotentialLaneIds(ego_lane_id_, common::LateralBehavior::kLaneChangeLeft, &potential_lcl_lane_ids_);
    GetPotentialLaneIds(ego_lane_id_, common::LateralBehavior::kLaneChangeRight, &potential_lcr_lane_ids_);
    return kSuccess;
}

ErrorType BehaviorPlanner::GetPotentialLaneIds(const int source_lane_id, const LateralBehavior& beh,
                                               std::vector<int>* candidate_lane_ids) {
    candidate_lane_ids->clear();
    if (beh == common::LateralBehavior::kUndefined || beh == common::LateralBehavior::kLaneKeeping) {
        map_itf_->GetChildLaneIds(source_lane_id, candidate_lane_ids); // ego_line的childs
    } else if (beh == common::LateralBehavior::kLaneChangeLeft) {
        int l_lane_id;
        if (map_itf_->GetLeftLaneId(source_lane_id, &l_lane_id) == kSuccess) {
            map_itf_->GetChildLaneIds(l_lane_id, candidate_lane_ids); // l_lane的childs
        ...
        if (map_itf_->GetRightLaneId(source_lane_id, &r_lane_id) == kSuccess) {
            map_itf_->GetChildLaneIds(r_lane_id, candidate_lane_ids); // r_lane的childs
...
std::vector<BehaviorPlanner::LateralBehavior> BehaviorPlanner::forward_behaviors() const {
    return forward_behaviors_;
}

int BehaviorPlanner::autonomous_level() const {
    return autonomous_level_;
}
