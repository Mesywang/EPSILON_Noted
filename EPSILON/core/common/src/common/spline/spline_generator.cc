...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetCubicSplineBySampleInterpolation(const vec_Vecf<N_DIM>& samples,
    ...
    for (int i = 1; i < num_samples; i++) {
        decimal_t dis = para[i] - para[i - 1]; // 每两个采样点的间距
        if (dis
            > para_delta_threshold) { // 若两个采样点的间距过大, 在拟合时可能会导致局部最优, 因此在两个采样点之间再插入几个点
            ...
            for (int n = 0; n < n_inserted; n++) {
                ...
                if (para_insert < para[i]) { // 防止para[i]被重复push_back
    ...
    for (int i = 0; i < N_DIM; i++) { // 循环每一维度
        ...
        cubic_fitting.set_points(
            X, Y); // X是累计的采样点一轴坐标, Y是采样点的一轴坐标, 引入interpolated_para来作为x轴和y轴的自变量

        for (int n = 0; n < cubic_fitting.num_pts() - 1; n++) { // 循环每一段轨迹
            Vecf<N_DEG + 1> coeff = Vecf<N_DEG + 1>::Zero(); // 系数个数 = 次数 + 1
            for (int d = 0; d <= N_DEG; d++) {
                if (d <= 3) {
                    coeff[N_DEG - d] = cubic_fitting.get_coeff(n, d) * fac(d); // why *fac(d)?
                } else {
                    coeff[N_DEG - d] = 0.0; // spline库只能求三次样条, 所以4 5次项的系数为0
                }
            }
            cubic_spline(n, i).set_coeff(coeff); // (segment, dim)
        }
    }
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetQuinticSplineBySampleFitting(const vec_Vecf<N_DIM>& samples,
                                                                         const std::vector<decimal_t>& para,
    ...
    Eigen::SparseMatrix<double, Eigen::RowMajor> C(N_DIM * num_connections * num_continuity,
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetWaypointsFromPositionSamples(const vec_Vecf<N_DIM>& samples,
                                                                         const std::vector<decimal_t>& para,
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromStateVec(const std::vector<decimal_t>& para,
                                                               const vec_E<State>& state_vec, SplineType* spline) {
    if (N_DEG < 5) {
    ...
    for (int i = 0; i < num_pts - 1; i++) {
        ...
        for (int d = 0; d < N_DIM; d++) {
            if (d <= 1) {
                (*spline)(i, d).GetJerkOptimalConnection(s0.position[d], s0.velocity[d], s0.acceleration[d],
...
template <int N_DEG, int N_DIM>
ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromFreeStateVec(const std::vector<decimal_t>& para,
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetBezierSplineUsingCorridor(
    const vec_E<SpatioTemporalSemanticCubeNd<N_DIM>>& cubes, const vec_E<Vecf<N_DIM>>& start_constraints,
    const vec_E<Vecf<N_DIM>>& end_constraints, const std::vector<decimal_t>& ref_stamps,
    const vec_E<Vecf<N_DIM>>& ref_points, const decimal_t& weight_proximity, BezierSplineType* bezier_spline) {
    int num_segments = static_cast<int>(cubes.size());
    int num_order = N_DEG + 1; // num_order = 6
    int derivative_degree = 3; // minimum jerk

    // ~ Stage I: stack objective, Matrix Q
    int total_num_vals = N_DIM * num_segments * num_order; // 优化变量的个数(所有控制点个数)
    Eigen::SparseMatrix<double, Eigen::RowMajor> Q(total_num_vals, total_num_vals);
    // SparseMatrix和reserve用法: https://www.licc.tech/article?id=22
    // 通过reserve方法，制定SparseMatrix每行(列)的最大非0元素个数
    Q.reserve(Eigen::VectorXi::Constant(N_DIM * num_segments * num_order, num_order));

    {
        MatNf<N_DEG + 1> hessian = BezierUtils<N_DEG>::GetBezierHessianMat(derivative_degree); // MtQM
        int idx, idy;
        decimal_t val;
        for (int n = 0; n < num_segments; n++) { // 段数
            decimal_t duration = cubes[n].t_ub - cubes[n].t_lb; // 当前段贝塞尔曲线的时间间隔, s = tj+1 - tj
            for (int d = 0; d < N_DIM; d++) { // 维度: x, y
                for (int j = 0; j < num_order; j++) { // 行
                    for (int k = 0; k < num_order; k++) { // 列
                        idx = d * num_segments * num_order + n * num_order + j;
                        idy = d * num_segments * num_order + n * num_order + k;
                        val = hessian(j, k) / pow(duration, 2 * derivative_degree - 3); // 1/s^(2*k-3)为放缩系数
    ...
    if (!ref_stamps.empty()) {
        ...
        for (int i = 0; i < num_ref_samples; i++) {
            if (ref_stamps[i] < cubes[0].t_lb || ref_stamps[i] > cubes[num_segments - 1].t_ub)
                continue; // ref_stamps不在corridor的时间范围内

            int n;
            for (n = 0; n < num_segments; n++) {
                if (cubes[n].t_ub > ref_stamps[i]) {
                    break; // 找到当前ref_point所对应的段数
                }
            }
            n = std::min(num_segments - 1, n);
            decimal_t s = cubes[n].t_ub - cubes[n].t_lb; // 当前段轨迹总时间间隔
            decimal_t t = ref_stamps[i] - cubes[n].t_lb; // 当前ref_point的时间间隔
            for (int d = 0; d < N_DIM; d++) {
                for (int j = 0; j < num_order; j++) {
                    idx = d * num_segments * num_order + n * num_order + j;
                    // 这里 += 表示同一段轨迹可能对应多个ref_points, 因此需要叠加
                    c[idx] += // 是想表示贝赛尔曲线的点和参考点的偏差? 写错了阿, 应该用"-"表示差异吧
                        -2 * ref_points[i][d] * (s * nchoosek(N_DEG, j) * pow(t / s, j) * pow(1 - t / s, N_DEG - j));
                    for (int k = 0; k < num_order; k++) {
                        idy = d * num_segments * num_order + n * num_order + k;
                        P.coeffRef(idx, idy) += s * s * nchoosek(N_DEG, j) * nchoosek(N_DEG, k) * pow(t / s, j + k)
    ...
    P = P * weight_proximity; // weight_proximity = 1.0
    ...
    // * 计算等式约束个数
    int num_continuity = 3; // 3阶导数连续
    int num_connections = num_segments - 1; // 连接点个数
    int num_continuity_constraints = N_DIM * num_connections * num_continuity; // 所有连续性约束的个数
    int num_start_eq_constraints = static_cast<int>(start_constraints.size()) * N_DIM; // 起点约束的个数: (p, v, a) * 2
    int num_end_eq_constraints = static_cast<int>(end_constraints.size()) * N_DIM; // 终点约束的个数: (p, v) * 2
    ...
    decimal_t val;
    {
        ...
        for (int n = 0; n < num_connections; n++) { // 连接点个数
            decimal_t duration_l = cubes[n].t_ub - cubes[n].t_lb;
            decimal_t duration_r = cubes[n + 1].t_ub - cubes[n + 1].t_lb;
            for (int c = 0; c < num_continuity; c++) { // 连续的阶数: num_continuity = 3, (pos, vel, acc)
                decimal_t scale_l = pow(duration_l, 1 - c); // pos -> s, vel -> 1, acc -> 1/s
                decimal_t scale_r = pow(duration_r, 1 - c);
                for (int d = 0; d < N_DIM; d++) {
                    idx = d * num_connections * num_continuity + n * num_continuity + c; // 对应某个约束
                    if (c == 0) {
                        // ~ position end
                        idy = d * num_segments * num_order + n * num_order + N_DEG; // 对应某个控制点
                        val = 1.0 * scale_l;
                        A.insert(idx, idy) = val;
                        // ~ position begin
                        ...
                        A.insert(idx, idy) = val;
                        idy = d * num_segments * num_order + n * num_order + N_DEG - 1;
                        val = -2.0 * scale_l; // 应该是42, -48, 42, 化简成1, -2, 1
                        A.insert(idx, idy) = val;
                        idy = d * num_segments * num_order + n * num_order + N_DEG;
                        val = 1.0 * scale_l;
                        A.insert(idx, idy) = val;
        ...
        {
            int num_order_constraint_start = static_cast<int>(start_constraints.size()); // 3, p v a
            decimal_t duration = cubes[0].t_ub - cubes[0].t_lb; // 第一段轨迹的缩放因子s
            decimal_t scale;
            int n = 0; // 第一段轨迹
            for (int j = 0; j < num_order_constraint_start; j++) { // p v a
                scale = pow(duration, 1 - j); // pos -> s, vel -> 1, acc -> 1/s
                for (int d = 0; d < N_DIM; d++) {
                    idx = num_continuity_constraints + d * num_order_constraint_start + j;
                    if (j == 0) { // pos
                        idy = d * num_segments * num_order + n * num_order + 0; // n = 0
                        val = 1.0 * scale; // scale = s, 位置控制点: ci
                        A.insert(idx, idy) = val;
                        ...
                        val = -1.0 * N_DEG * scale; // scale = 1
                        A.insert(idx, idy) = val;
                        idy = d * num_segments * num_order + n * num_order + 1;
                        val = 1.0 * N_DEG * scale; // 速度控制点: n*(ci+1-ci)
                        A.insert(idx, idy) = val;
                        ...
                        idy = d * num_segments * num_order + n * num_order + 0;
                        val = 1.0 * N_DEG * (N_DEG - 1) * scale; // scale = 1 / s
                        A.insert(idx, idy) = val;
                        ...
                        val = 1.0 * N_DEG * (N_DEG - 1) * scale; // 加速度控制点:  n*(n-1)*(ci+2-2*ci+1-ci)
                        A.insert(idx, idy) = val;
        ...
        {
            int num_order_constraint_end = static_cast<int>(end_constraints.size()); // 2, p v
            decimal_t duration = cubes[num_segments - 1].t_ub - cubes[num_segments - 1].t_lb; // 最后一段轨迹的缩放因子s
            decimal_t scale;
            int n = num_segments - 1; // 倒数第二段轨迹
    ...
    for (int i = 0; i < num_segments; i++) { // num_segments = cubes.size()
        total_num_ineq += (static_cast<int>(cubes[i].p_ub.size())) * num_order; // 错了吧? 没乘维度N_DIM
    ...
    Eigen::VectorXd lbd;
    Eigen::VectorXd ubd;
    Eigen::SparseMatrix<double, Eigen::RowMajor> C(total_num_ineq, total_num_vals); // 约束个数*控制点个数
    ...
    ubd.setZero(total_num_ineq);
    {
        ...
        for (int n = 0; n < num_segments; n++) {
            ...
            for (int d = 0; d < N_DIM; d++) {
                ...
                scale = pow(duration, 1 - 1); // 1
                for (int j = 0; j < num_order - 1; j++) {
                    idx = accu_num_ineq;
                    idy = d * num_segments * num_order + n * num_order + j;
                    val = -N_DEG * scale; // 速度控制点: n*(ci+1-ci)
                    C.insert(idx, idy) = val;
                ...
                scale = pow(duration, 1 - 2); // 1 / s
                for (int j = 0; j < num_order - 2; j++) {
                    idx = accu_num_ineq;
                    idy = d * num_segments * num_order + n * num_order + j;
                    val = N_DEG * (N_DEG - 1) * scale; // 加速度控制点:  n*(n-1)*(ci+2-2*ci+1-ci)
                    C.insert(idx, idy) = val;
                    idy = d * num_segments * num_order + n * num_order + (j + 1);
                    val = -2.0 * N_DEG * (N_DEG - 1) * scale;
                    C.insert(idx, idy) = val;
    ...
    vec_domain.push_back(cubes.front().t_lb); // 初始时间
    for (int n = 0; n < num_segments; n++) {
        vec_domain.push_back(cubes[n].t_ub); // t0, t1, t2, ... , tn
    }
    bezier_spline->set_vec_domain(vec_domain);
    for (int n = 0; n < num_segments; n++) {
        for (int j = 0; j < num_order; j++) {
            Vecf<N_DIM> coeff; // C_x, C_y
            for (int d = 0; d < N_DIM; d++)
