...
public:
    ...
    void set_vec_domain(const std::vector<decimal_t>& vec_domain) {
        ...
        ctrl_pts_.resize(vec_domain.size() - 1); // 相当于轨迹的段数
        for (int j = 0; j < static_cast<int>(ctrl_pts_.size()); j++) {
            ctrl_pts_[j].setZero(); // Matrix.setZero()
        }
    }

    void set_coeff(const int segment_idx, const int ctrl_pt_index, const Vecf<N_DIM>& coeff) {
    ...
    int num_segments() const {
        return static_cast<int>(ctrl_pts_.size());
    }
     ...
     */
    vec_E<Matf<N_DEG + 1, N_DIM>> ctrl_pts() const {
        return ctrl_pts_;
    }
    ...
    ErrorType evaluate(const decimal_t s, const int d, Vecf<N_DIM>* ret) const {
        ...
        // lower_bound()在first和last中的前闭后开区间, 进行二分查找, 返回从first开始的第一个大于或等于val的元素的地址
        // 如果所有元素都小于val, 则返回last的地址
        auto it = std::lower_bound(vec_domain_.begin(), vec_domain_.end(), s);
        int idx = std::min(std::max(static_cast<int>(it - vec_domain_.begin()) - 1, 0), num_pts - 2); // 上界
        decimal_t h = s - vec_domain_[idx];
        if (s < vec_domain_[0]) {
            return kIllegalInput;
        } else {
            decimal_t duration = vec_domain_[idx + 1] - vec_domain_[idx];
            decimal_t normalized_s = h / duration; // 标准贝塞尔曲线定义域[0, 1]
            // printf("derivative %d, normalized s: %lf.\n", d, normalized_s);
            Vecf<N_DEG + 1> basis = BezierUtils<N_DEG>::GetBezierBasis(d, normalized_s);
            // (pow(duration, 1 - d)为缩放因子s, 1, 1 / s
            Vecf<N_DIM> result = (pow(duration, 1 - d) * basis.transpose() * ctrl_pts_[idx]).transpose(); // 列向量
            *ret = result;
        }
        return kSuccess;
    }
            ...
            case 5:
                if (derivative_degree == 0) {
                    ...
                    basis << -5 * pow(t - 1, 4), 20 * t * pow(t - 1, 3) + 5 * pow(t - 1, 4),
                        ...
                        20 * pow(t, 3) * (t - 1) + 30 * pow(t, 2) * pow(t - 1, 2),
                    ...
                    basis << -60 * pow(t - 1, 2), 60 * t * (2 * t - 2) + 180 * pow(t - 1, 2),
                        ...
                        180 * t * (2 * t - 2) + 60 * pow(t - 1, 2) + 180 * t * t, -120 * t * (t - 1) - 180 * t * t,
                        60 * t * t;
                }
                break;
            default:
