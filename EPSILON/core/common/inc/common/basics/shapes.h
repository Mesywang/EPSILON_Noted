...
struct OrientedBoundingBox2D {
    ...
    /**
     * @brief Construct a new Oriented Bounding Box 2D object
     *
     ...
     */
    OrientedBoundingBox2D(const decimal_t x_, const decimal_t y_, const decimal_t angle_, const decimal_t width_,
...
template <int N> struct AxisAlignedBoundingBoxND {
     ...
     */
    AxisAlignedBoundingBoxND(const std::array<decimal_t, N> coord_, const std::array<decimal_t, N> len_)
        : coord(coord_),
          len(len_) {}
};
...
template <typename T, int N_DIM> struct AxisAlignedCubeNd {
    ...
    AxisAlignedCubeNd(const std::array<T, N_DIM> ub, const std::array<T, N_DIM> lb)
        : upper_bound(ub),
          lower_bound(lb) {}
};
...
public:
     ...
     */
    static bool CheckIfOrientedBoundingBoxIntersect(const OrientedBoundingBox2D& obb_a,
     ...
     * @return ErrorType
     */
    static ErrorType GetPerpendicularAxisOfOrientedBoundingBox(const vec_E<Vecf<2>>& vertices, const int index,
                                                               Vecf<2>* axis);
     ...
     * @return ErrorType
     */
    static ErrorType GetCvPoint2iVecUsingCommonPoint2iVec(const std::vector<Point2i>& pts_in,
     ...
     */
    template <typename T, int N_DIM>
    static bool CheckIfAxisAlignedCubeCollide(const common::AxisAlignedCubeNd<T, N_DIM>& aabb_a,
     ...
     */
    template <typename T, int N_DIM>
    static bool CheckIfAxisAlignedCubeNdIntersect(const AxisAlignedCubeNd<T, N_DIM>& aabb_a,
     ...
     */
    template <typename T, int N_DIM>
    static bool CheckIfAxisAlignedCubeNdIntersectionOnOneDim(const AxisAlignedCubeNd<T, N_DIM>& aabb_a,
     ...
     */
    template <typename T, int N_DIM>
    static bool CheckIfAxisAlignedCubeNdCollideOnOneDim(const AxisAlignedCubeNd<T, N_DIM>& aabb_a,
                                                        const AxisAlignedCubeNd<T, N_DIM>& aabb_b, const int& i) {
        decimal_t half_len_a = fabs(static_cast<double>(aabb_a.upper_bound[i] - aabb_a.lower_bound[i]) / 2.0);
        decimal_t half_len_b = fabs(static_cast<double>(aabb_b.upper_bound[i] - aabb_b.lower_bound[i]) / 2.0);
