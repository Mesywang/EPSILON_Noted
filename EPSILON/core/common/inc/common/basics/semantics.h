...
private:
    double width_ = 1.90; // y axis
    double length_ = 4.88; // x axis
...
struct SemanticBehavior {
    ...
    vec_E<vec_E<Vehicle>> forward_trajs; // 每种行为对应的ego_car的前向轨迹
    std::vector<LateralBehavior> forward_behaviors; // 所有可能的行为
    vec_E<std::unordered_map<int, vec_E<Vehicle>>> surround_trajs; // 每种行为对应的周围所有车的预测轨迹
...
public:
    ...
    inline T data(const int& i) const {
        return data_[i]; // 访问地图元素
    }
    inline T* get_data_ptr() {
        return data_.data(); // 在c++11中, vector增加了data()的用法，它返回内置vecotr所指的数组内存的第一个元素的指针。
    }
    ...
    inline void set_dims_resolution(const std::array<decimal_t, N_DIM>& dims_resolution) {
     ...
     * @return ErrorType
     */
    ErrorType CheckIfEqualUsingGlobalPosition(const std::array<decimal_t, N_DIM>& p_w, const T& val_in,
                                              bool* res) const;
     ...
     * @return ErrorType
     */
    ErrorType GetGlobalPositionUsingCoordinate(const std::array<int, N_DIM>& coord,
...
private:
    ...
    std::array<int, N_DIM> dims_size_; // size of each dimension
    std::array<int, N_DIM> dims_step_; // E.g. A x-y-z map's steps are {1, x, x*y}
    std::array<decimal_t, N_DIM> dims_resolution_; // resolution of each dimension
    std::array<std::string, N_DIM> dims_name_; // name of each dimension
    std::array<decimal_t, N_DIM> origin_; // origin of map

    int data_size_{0}; // size of all dimension
    std::vector<T> data_;
};
...
struct ObstacleSet {
     ...
     */
    inline int size() const {
        return obs_circle.size() + obs_polygon.size();
    }
...
template <int N_DIM> struct SpatioTemporalSemanticCubeNd {
    decimal_t t_lb, t_ub;
    std::array<decimal_t, N_DIM> p_lb, p_ub; // pos, {x, y}
    std::array<decimal_t, N_DIM> v_lb, v_ub; // vel, {x, y}
    std::array<decimal_t, N_DIM> a_lb, a_ub; // acc, {x, y}
    ...
    void FillDefaultBounds() {
        // ~ for optimization, infinity bound will cause numerical unstable.
        // ~ So we put some loose bounds by default.
        t_lb = 0.0;
        t_ub = 1.0;
...
public:
     ...
     * @return ErrorType
     */
    static ErrorType GetOrientedBoundingBoxForVehicleUsingState(const VehicleParam& param, const State& s,
    ...
    static ErrorType InflateVehicleBySize(const Vehicle& vehicle_in, const decimal_t delta_w, const decimal_t delta_l,
                                          Vehicle* vehicle_out);
