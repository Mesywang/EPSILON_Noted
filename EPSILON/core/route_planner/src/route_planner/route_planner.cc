...
ErrorType RoutePlanner::NaviLoopRandomExpansion() {
    switch (navi_status_) {
        case kReadyToGo:
            // 每3个周期调用一次GetNaviPathByRandomExpansion
...
ErrorType RoutePlanner::GetNaviPathByRandomExpansion() {
    ...
    // 在childs中通过随机数随机选择一个id作为route
    while (dist_acc < navi_path_max_length_) { // navi_path_max_length_ = 200
    ...
    for (const auto& id : navi_path_) {
        if (raw_samples.empty() && (int)lane_net_.lane_set.at(id).lane_points.size() > 0) {
            raw_samples.push_back(lane_net_.lane_set.at(id).lane_points[0]);
        }
        for (int i = 1; i < (int)lane_net_.lane_set.at(id).lane_points.size(); ++i) {
            raw_samples.push_back(lane_net_.lane_set.at(id).lane_points[i]);
        }
    }

    if (common::LaneGenerator::GetLaneBySamplePoints(raw_samples, &navi_lane_) != kSuccess) {
    ...
    if (navi_lane_.GetArcLengthByVecPosition(pos, &navi_start_arc_length_) != kSuccess) {
...
ErrorType RoutePlanner::GetChildLaneIds(const int lane_id, std::vector<int>* child_ids) {
