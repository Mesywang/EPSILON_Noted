...
public:
    ...
    struct Config {
        ...
        void Print() {
            ...
            printf(" -- map_resolution: [%lf, %lf, %lf]\n", map_resolution[0], map_resolution[1], map_resolution[2]);
            printf(" -- axis_name: [%s, %s, %s]\n", axis_name[0].c_str(), axis_name[1].c_str(), axis_name[2].c_str());
            ...
            printf(" -- inflate_steps: [%d, %d, %d, %d, %d, %d]\n", inflate_steps[0], inflate_steps[1],
                   inflate_steps[2], inflate_steps[3], inflate_steps[4], inflate_steps[5]);
        }
    };
    ...
    vec_E<vec_E<common::SpatioTemporalSemanticCubeNd<2>>> final_corridor_vec() const {
    ...
    ErrorType ConstructSscMap(const std::unordered_map<int, vec_E<common::FsVehicle>>& sur_vehicle_trajs_fs,
                              const vec_E<Vec2f>& obstacle_grids);

    ErrorType InflateObstacleGrid(const common::VehicleParam& param);

    ErrorType ConstructCorridorUsingInitialTrajectory(GridMap3D* p_grid, const vec_E<common::FsVehicle>& trajs);
...
private:
    bool CheckIfCubeIsFree(GridMap3D* p_grid, const common::AxisAlignedCubeNd<int, 3>& cube) const;

    bool CheckIfPlaneIsFreeOnXAxis(GridMap3D* p_grid, const common::AxisAlignedCubeNd<int, 3>& cube,
                                   const int& z) const;

    bool CheckIfPlaneIsFreeOnYAxis(GridMap3D* p_grid, const common::AxisAlignedCubeNd<int, 3>& cube,
                                   const int& z) const;

    bool CheckIfPlaneIsFreeOnZAxis(GridMap3D* p_grid, const common::AxisAlignedCubeNd<int, 3>& cube,
                                   const int& z) const;

    bool CheckIfCubeContainsSeed(const common::AxisAlignedCubeNd<int, 3>& cube_a, const Vec3i& seed) const;

    ErrorType GetInitialCubeUsingSeed(const Vec3i& seed_0, const Vec3i& seed_1,
                                      common::AxisAlignedCubeNd<int, 3>* cube) const;

    ErrorType GetTimeCoveredCubeIndices(const common::DrivingCorridor* p_corridor, const int& start_id, const int& dir,
                                        const int& t_trans, std::vector<int>* idx_list) const;

    ErrorType CorridorRelaxation(GridMap3D* p_grid, common::DrivingCorridor* p_corridor);

    ErrorType InflateCubeIn3dGrid(GridMap3D* p_grid, const std::array<bool, 6>& dir_disabled,
                                  const std::array<int, 6>& dir_step, common::AxisAlignedCubeNd<int, 3>* cube);

    ErrorType GetInflationDirections(const bool& if_first_cube, std::array<bool, 6>* dirs_disabled);

    bool InflateCubeOnXPosAxis(GridMap3D* p_grid, const int& n_step, common::AxisAlignedCubeNd<int, 3>* cube);
    bool InflateCubeOnXNegAxis(GridMap3D* p_grid, const int& n_step, common::AxisAlignedCubeNd<int, 3>* cube);
    bool InflateCubeOnYPosAxis(GridMap3D* p_grid, const int& n_step, common::AxisAlignedCubeNd<int, 3>* cube);
    bool InflateCubeOnYNegAxis(GridMap3D* p_grid, const int& n_step, common::AxisAlignedCubeNd<int, 3>* cube);
    bool InflateCubeOnZPosAxis(GridMap3D* p_grid, const int& n_step, common::AxisAlignedCubeNd<int, 3>* cube);
    bool InflateCubeOnZNegAxis(GridMap3D* p_grid, const int& n_step, common::AxisAlignedCubeNd<int, 3>* cube);

    ErrorType FillStaticPart(const vec_E<Vec2f>& obs_grid_fs);

    ErrorType FillDynamicPart(const std::unordered_map<int, vec_E<common::FsVehicle>>& sur_vehicle_trajs_fs);
    ...
    // first vec -> muti behaviors, second vec -> muti cubes in one corridor
    vec_E<vec_E<common::SpatioTemporalSemanticCubeNd<2>>> final_corridor_vec_; // Nd = 2: 因为优化时不需要考虑时间维度了
};
