...
public:
     ...
     * @brief getters
     */
    SscMap* p_ssc_map() const {
        return p_ssc_map_;
    }

    FrenetState ego_frenet_state() const {
        return ego_frenet_state_;
    }

    Vehicle ego_vehicle() const {
        return ego_vehicle_;
    }

    vec_E<vec_E<Vehicle>> forward_trajs() const {
        return forward_trajs_;
    }

    vec_E<BezierSpline> qp_trajs() const {
        return qp_trajs_;
    }

    decimal_t time_origin() const {
        return time_origin_;
    }

    std::unordered_map<int, vec_E<common::FsVehicle>> sur_vehicle_trajs_fs() const {
        return sur_vehicle_trajs_fs_;
    }

    vec_E<std::unordered_map<int, vec_E<common::FsVehicle>>> surround_forward_trajs_fs() const {
    ...
    common::FsVehicle fs_ego_vehicle() const {
        return fs_ego_vehicle_;
    }

    // BezierSpline bezier_spline() const { return bezier_spline_; }

    std::unique_ptr<FrenetTrajectory> trajectory() const {
        if (!is_lateral_independent_) { // 低速
            return std::unique_ptr<FrenetPrimitiveTrajectory>(new FrenetPrimitiveTrajectory(low_spd_alternative_traj_));
        }
        return std::unique_ptr<FrenetBezierTrajectory>(new FrenetBezierTrajectory(trajectory_)); // 高速
    }

    common::StateTransformer state_transformer() const {
        return stf_;
    }
...
private:
    ...
    ErrorType CorridorFeasibilityCheck(const vec_E<common::SpatioTemporalSemanticCubeNd<2>>& cubes);
    /**
     ...
     */
    ErrorType StateTransformUsingOpenMp(const vec_E<State>& global_state_vec, const vec_E<Vec2f>& global_point_vec,
                                        vec_E<FrenetState>* frenet_state_vec, vec_E<Vec2f>* fs_point_vec) const;

    ErrorType StateTransformSingleThread(const vec_E<State>& global_state_vec, const vec_E<Vec2f>& global_point_vec,
                                         vec_E<FrenetState>* frenet_state_vec, vec_E<Vec2f>* fs_point_vec) const;
    ...
    // Map
    SscPlannerMapItf* map_itf_; // map_itf_是map_adapter_的父类
