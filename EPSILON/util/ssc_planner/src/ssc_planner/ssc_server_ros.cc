...
SscPlannerServer::SscPlannerServer(ros::NodeHandle nh, int ego_id) : nh_(nh), work_rate_(20.0), ego_id_(ego_id) {
...
SscPlannerServer::SscPlannerServer(ros::NodeHandle nh, double work_rate, int ego_id)
    : nh_(nh),
...
void SscPlannerServer::PublishData() {
    ...
    // trajectory feedback
    {
        ...
        if (use_sim_state_) {
            ...
            decimal_t ct = executing_traj_->begin() + num_cycles * plan_horizon; // 将t与周期间隔对齐
            {
                common::State state;
                if (executing_traj_->GetState(ct, &state) == kSuccess) {
                    ...
                    if (ctrl_state_hist_.size() > 100)
                        ctrl_state_hist_.erase(ctrl_state_hist_.begin()); // 删除历史轨迹
                    vehicle_msgs::ControlSignal ctrl_msg;
                    vehicle_msgs::Encoder::GetRosControlSignalFromControlSignal(
                        common::VehicleControlSignal(state), ros::Time(ct), std::string("map"), &ctrl_msg);
                    ctrl_signal_pub_.publish(ctrl_msg); // 发布控制指令
                } else {
                    printf("[SscPlannerServer]cannot evaluate state at %lf with begin "
                           "%lf.\n",
        ...
        // trajectory visualization
        {
            ...
            common::VisualizationUtil::GetMarkerArrayByTrajectory(*executing_traj_, 0.1, Vecf<3>(0.3, 0.3, 0.3), color,
                                                                  0.5, &traj_mk_arr);
            if (require_intervention_signal_) {
                ...
                common::VisualizationUtil::GetRosMarkerTextUsingPositionAndString(
                    pos, std::string("Intervention Needed!"), common::cmap["red"], Vec3f(5.0, 5.0, 5.0), 0,
                    &traj_status);
                traj_mk_arr.markers.push_back(traj_status);
            }
            int num_traj_mks = static_cast<int>(traj_mk_arr.markers.size());
            common::VisualizationUtil::FillHeaderIdInMarkerArray(ros::Time(current_time), std::string("/map"),
                                                                 last_trajmk_cnt_, &traj_mk_arr);
...
ErrorType SscPlannerServer::FilterSingularityState(const vec_E<common::State>& hist, common::State* filter_state) {
    if (hist.empty()) {
        return kWrongStatus;
    }
    ...
    if (fabs(filter_state->velocity) < singular_velocity
        && fabs(normalize_angle(filter_state->angle - hist.back().angle)) > max_orientation_change) {
        printf("[SscPlannerServer]Detect singularity velocity %lf angle (%lf, %lf).\n", filter_state->velocity,
               hist.back().angle, filter_state->angle);
...
void SscPlannerServer::Init(const std::string& config_path) {
    planner_.Init(config_path); // 初始化规划器所用的各种参数
    ...
    map_marker_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("ssc_map", 1);
    executing_traj_vis_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(traj_topic, 1);
}

void SscPlannerServer::Start() {
    if (is_replan_on_) {
        return; // 保证线程MainThread只创建一次
    }
    planner_.set_map_interface(&map_adapter_); // planner_.map_itf_是map_adapter_的父类
...
void SscPlannerServer::MainThread() {
    ...
    const milliseconds interval{static_cast<int>(1000.0 / work_rate_)}; // 1000 / 20
    while (true) {
...
void SscPlannerServer::PlanCycleCallback() {
    ...
    while (p_input_smm_buff_->try_dequeue(last_smm_)) { // 弹出最新的SemanticMapManager
    ...
    map_adapter_.set_map(map_ptr); // map_adapter_是planner_.map_itf_的子类
    ...
    if (executing_traj_ == nullptr || !executing_traj_->IsValid()
        || !use_sim_state_) { // executing_traj_:std::unique_ptr
        // Init planning, only execute once
        ...
        desired_state_hist_.push_back(last_smm_.ego_vehicle().state()); // 状态历史
        ctrl_state_hist_.clear();
        ctrl_state_hist_.push_back(last_smm_.ego_vehicle().state()); // 控制历史
        executing_traj_ = std::move(planner_.trajectory()); // 执行的轨迹
        global_init_stamp_ = executing_traj_->begin(); // 轨迹的时间起点
        printf("[SscPlannerServer]init plan success with stamp: %lf and angle %lf.\n", global_init_stamp_,
               last_smm_.ego_vehicle().state().angle);
        return;
    }

    if (current_time > executing_traj_->end()) { // 任务完成
        printf("[SscPlannerServer]Current time %lf out of [%lf, %lf].\n", current_time, executing_traj_->begin(),
               executing_traj_->end());
        printf("[SscPlannerServer]Mission complete.\n");
        // is_replan_on_ = false;
        // .release()时不会释放指针所指的内存, 这时返回值就是对这块内存的唯一索引
        executing_traj_.release(); // 内存泄漏?
        next_traj_.release();
        return;
    }
...
void SscPlannerServer::Replan() {
    ...
    printf("[SscPlannerServer]init stamp: %lf, plan horizon: %lf, num cycles %d.\n", global_init_stamp_, plan_horizon,
           num_cycles_ahead);
    ...
    printf("[SscPlannerServer]t %lf, desired state (x,y,v,a,theta):(%lf, %lf, %lf, "
           "%lf, %lf).\n",
           t, desired_state.vec_position[0], desired_state.vec_position[1], desired_state.velocity,
           desired_state.acceleration, desired_state.angle);
